...
>
> 插件 v1.7.0 起的"模型用量倍率"显示需要服务端 ≥ **2026-09-19 上游版**（`/v1/stats`
> 每模型透出上游积分倍率 `credits`，上游 5009a1f）；旧版服务端该字段缺失，tooltip 与
> 成本台账弹窗只是不显示倍率，其余照常。
>
> 插件 v1.8.0 起的"任务队列"定时排程行需要服务端 ≥ **2026-09-21 合并版**（调度器
> 新增 `queue` 任务类型，`/admin/tasks` 透出 7 类；旧版服务端该行不出现，其余功能
> 不受影响）。
...
- **设置（5 页）**：服务（目录/端口/启停/自启）、账户与积分（实时列含"剩余（已用/总量）"；
  ...
  每1k=实测千 token 均价、≤0 即免费，同 wb2api `status-report.ps1` 口径；**"悬浮窗"列**
  实时反映该号是否出现在悬浮提示明细里，与右键菜单的"悬浮提示显示此账户"同一份开关）、
  定时任务（勾选=热生效；触发时间可直接编辑并「应用」写回服务 config.json，重启服务后生效；
  立即执行=服务进程内触发；**任务队列**行=对齐网页端任务中心的"执行队列定时排程"，
  默认关、勾选启用，扫描全账号待办并排队执行，见下文）、显示、高级。
- **插件命令菜单**：启动/停止/重启、查询实时积分、立即执行/启用各类任务（含任务队列）、三个开关。
- **任务队列**（需服务端 ≥ 2026-09-21 合并版，`admin.enabled`）：对齐网页端任务中心的
  "执行队列"——扫描全部账号的待办（成长任务 + 开学季闭环）并按账号排队执行。设置窗③
  新增第 7 行「任务队列」：**定时排程默认关**（队列对全账号执行真实任务动作链、消耗上游
  配额，同网页端 opt-in 口径），勾选即热启用并写回 `schedule.queue_enabled`；触发时间
  默认 10 点（`queue_hours`），可编辑并「应用」写回；「立即执行」不等排程、随时可手动跑
  一次（撞车 409 提示，与网页端互斥语义一致）。命令菜单同步提供"立即执行: 任务队列"与
  "启用: 任务队列"两项。
...
- **按模型用量统计**（需服务端 ≥ 2026-09-20 fork 版，`GET /v1/stats`）：tooltip"模型用量"
  ...
  同时展示该号的单价台账与全局按模型消耗两块。服务端 ≥ 2026-09-19 上游版（5009a1f）
  还会在每行缀上游积分倍率（如"（x0.06）"，与 /v1/models 同源；目录未下发时整体省略，
  缺失≠免费）。
- **悬浮窗账户只显实时积分**（v1.9.0）：tooltip 账户区节标题改为"账户"，行内不再显示
  本地估算值——估算随轮询单调扣减、长期偏差大，小字号悬浮窗宁可少也不误导；实时缓存
  缺失时该行不显示积分数，需要精确值去设置窗账户表（"查询实时积分"）。账户表与栏位
  合计的显示口径不变。
...
**悬浮提示**（完整展开模式）——状态、账户（实时积分）、定时任务、积分汇总：
...
- 小程序成长任务（minichat）刻意**不在**"定时任务"页出现：它不进 wb2api 的 taskKind 枚举、
  ...
  任务队列（queue）则相反：它**是**调度器第 7 类任务（2026-09-21 合并版起），故 v1.8.0
  起在"定时任务"页有独立一行，排程开关缺省 false 与服务端默认一致。
...
- v1.6.0 起本插件对自家 tooltip 上**硬预算 700 字符**（`worker.cpp` 的 `kTipBudget`）：完整展开
  （默认）在原版形态（实测 8 账号+7 任务+模型用量 = 约 780 字符）下最多舍弃最末的"模型用量"区，
  ...
  - **"悬浮提示显示定时任务明细"** 开关（`tooltip_tasks`，**v1.7.0 起默认关闭**）：7 任务
    × 20+ 字符是次要区里最长且变化最频繁的，多数时间没有观测价值；需要盯任务下次触发
    时刻的再打开。关闭后健康概要等汇总行不受影响，任务详情仍看设置窗③与插件命令菜单；
  ...
  - **单账户显隐**：设置②"账户与积分"页**右键账户行**勾/去"悬浮提示显示此账户"
    ...
    v1.7.0 起账户表新增**"悬浮窗"列**（是/否）直观反映该开关的当前状态；
